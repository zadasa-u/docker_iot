            ...
            async for message in messages:
                ...
                if message.topic.matches("sensores_remotos/#"):
                    ...
                    async with conn.cursor() as cur:
                        try:
                            await cur.execute(sql, (str(message.topic).split('/')[1], datos['temperatura'], datos['humedad']))
                            await conn.commit()
                            await cur.close()
                ...
                if message.topic.matches("/stackberry/temperatura/+/cpu"):
                    logging.info(str(message.topic).split('/')[3]+ ": " +message.payload.decode('utf8'))
                    sql = "INSERT INTO `temp_cpus` (`cpu`, `temperatura`) VALUES (%s, %s)"
                    async with conn.cursor() as cur:
                        try:
                            await cur.execute(sql, (str(message.topic).split('/')[3], message.payload.decode('utf8')))
                            await conn.commit()
                            await cur.close()
                            await conn.ensure_closed()
                        except Exception as e:
