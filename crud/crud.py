...
def registrar():
    ...
    if request.method == "POST":
        ...
        passhash = generate_password_hash(
            request.form.get("password"), method="pbkdf2:sha256:10000", salt_length=16
        )
        cur = mysql.connection.cursor()
        cur.execute(
            "INSERT INTO usuarios (usuario, hash) VALUES (%s,%s)",
            (request.form.get("usuario"), passhash[20:]),
        )
    ...
    if request.method == "POST":
        ...
        if rows:
            if check_password_hash(
                "pbkdf2:sha256:10000$" + rows[2], request.form.get("password")
            ):
