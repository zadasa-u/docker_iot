...
logging.basicConfig(
    format="%(asctime)s - TelegramBot - %(levelname)s - %(message)s", level=logging.INFO
)
...
async def acercade(update: Update, context):
    await context.bot.send_message(
        update.message.chat.id, text="Este bot fue creado para el curso de IoT (2023)"
    )
...
async def kill(update: Update, context):
    ...
    if context.args and context.args[0] == "@e":
        await context.bot.send_animation(
            update.message.chat.id,
            "CgACAgEAAxkBAANXZAiWvDIEfGNVzodgTgH1o5z3_WEAAmUCAALrx0lEZ8ytatzE5X0uBA",
        )
        await asyncio.sleep(6)
        await context.bot.send_message(
            update.message.chat.id, text="¡¡¡Ahora estan todos muertos!!!"
        )
    else:
        await context.bot.send_message(
            update.message.chat.id, text="☠️ ¡¡¡Esto es muy peligroso!!! ☠️"
        )
...
async def medicion(update: Update, context):
    ...
    async with conn.cursor() as cur:
        ...
        await context.bot.send_message(
            update.message.chat.id,
            text="La última {} es de {} {},\nregistrada a las {:%H:%M:%S %d/%m/%Y}".format(
                update.message.text, str(r[1]).replace(".", ","), unidad, r[0]
            ),
        )
        logging.info(
            "La última {} es de {} {}, medida a las {:%H:%M:%S %d/%m/%Y}".format(
                update.message.text, r[1], unidad, r[0]
            )
        )
    conn.close()
...
def main():
    ...
    application.add_handler(
        MessageHandler(filters.Regex("^(temperatura|humedad)$"), medicion)
    )
    application.add_handler(
        MessageHandler(
            filters.Regex("^(gráfico temperatura|gráfico humedad)$"), graficos
        )
    )
